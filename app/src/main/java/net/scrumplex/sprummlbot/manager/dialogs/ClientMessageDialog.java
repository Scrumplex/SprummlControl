...
public class ClientMessageDialog extends DialogFragment {

    private JSONObject client;
    private String apiKey;
    private String baseUrl;

    public ClientMessageDialog() {
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        try {
            builder.setTitle(client.getString("client_nickname")).setView(dialogView).setPositiveButton("Send", new DialogInterface.OnClickListener() {
                @Override
                public void onClick(DialogInterface dialog, int which) {
                    final String msg = message.getText().toString();
                    try {
                        String request = baseUrl + "/clients/" + client.getString("clid") + "/sendmsg";
                        ...
                        new APIRequest(new APICallback() {
                            ...
                            public void handle(JSONObject response) throws JSONException {
                                if (response.getString("msg").equalsIgnoreCase("success")) {
                                    Toast.makeText(c, "Message was sent!", Toast.LENGTH_SHORT).show();
                                } else {
                                    Toast.makeText(c, "Message could not be sent!", Toast.LENGTH_SHORT).show();
                                }
                            }
                        }).execute(data);
